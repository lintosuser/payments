  ...
  try {
    ...
    if (result.success) {
      await dbExec`
        ...
        SET status = 'approved', ccode = ${result.responseCode},
            acode = ${result.confirmationCode || tx.acode || ""}
        WHERE yaad_id = ${transId}`;
    }

    await logAudit({
      user, action: "payment.commit", ip: getIp(req),
    ...
    return NextResponse.json({
      ...
      message: result.success ? "העסקה אושרה" : provider.errorMessage(result.responseCode),
      result: result.raw,
    });
